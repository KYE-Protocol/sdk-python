# KYE Protocol™ — Python SDK

[![PyPI](https://img.shields.io/badge/pypi-kye--protocol-1A8754?style=flat-square)](https://pypi.org/project/kye-protocol/)
...
> The official Python SDK for **KYE Protocol™** — the open Entity Authority Protocol for AI Governance.
>
> Implements the public-vocabulary surface today (URN parsing, type constants, schema dataclasses). The full runtime client (decision endpoint, signing helpers, evidence-pack builder) ships with v1.0 GA.
...
# 1. Parse a KYE URN
urn = KyeUrn.parse("kye:agent:acme.eu:treasury:fx-router")
assert urn.entity_class == "agent"
assert urn.trust_domain == "acme.eu"
assert urn.subclass     == "treasury"
assert urn.local        == "fx-router"

# 2. Build a URN
card_token = KyeUrn.build(
    entity_class=EntityClass.CREDENTIAL,
    trust_domain="bank-z.eu",
    subclass="card-token",
    local="tok_abc123",
)
assert str(card_token) == "kye:credential:bank-z.eu:card-token:tok_abc123"
...
```bash
python -m kye_protocol.conformance
```
...
│       ├── urn.py          # KYE URN parser + builder
│       ├── vocabulary.py   # Entity classes, decision codes, signal types
│       └── types.py        # dataclasses for every public schema
├── tests/                  # pytest specs
├── pyproject.toml
├── LICENSE
└── README.md
